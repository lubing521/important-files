...
bool IsValidChar(const TCHAR *buf)
{
	...
	userlen=strlen(buf);
	for (int i=0;i<sizeof(ErrorChar);i++)
	{
...
BOOL DeleteGameFile(LPCTSTR lpszFileName)
{
...
BOOL DeleteGameDirectory(LPCTSTR lpPath)
{
	...
	if (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
	{
		if(!((FindFileData.cFileName[0]=='.' && FindFileData.cFileName[1]=='\0')||(FindFileData.cFileName[0]=='.' && FindFileData.cFileName[1]=='.' && FindFileData.cFileName[2]=='\0')))
		{
			...
			if (!DeleteGameDirectory(strPath.c_str()))
			{
	...
	else
	{
		...
		if (!DeleteGameFile(strFileName.c_str()))
		{
			bRet=FindClose(hFind);
			assert(bRet);
			return FALSE;
...
BOOL GetFileCrc32(LPCTSTR lpFileName,DWORD& dwCrc32)
{
	...
	dwCrc32=CalBufCRC32((unsigned char*)lpFileBuf,dwSize);
	//delete []lpFileBuf;
	I8MP.free(lpFileBuf);
	return TRUE;
}
