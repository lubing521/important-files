...
IMPLEMENT_DYNAMIC(CWizardAddGameBoard, CDialog)

CWizardAddGameBoard::CWizardAddGameBoard(CWnd* pParent /*=NULL*/)
	: CDialog(CWizardAddGameBoard::IDD, pParent)
...
CWizardAddGameBoard::~CWizardAddGameBoard()
{
	CloseHandle(m_hExited);
	delete[] m_pbuf;
}

void CWizardAddGameBoard::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CWizardAddGameBoard, CDialog)
	ON_BN_CLICKED(IDOK, &CWizardAddGameBoard::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BROWSE_DIR, &CWizardAddGameBoard::OnBnClickedBrowseDir)
	ON_BN_CLICKED(IDC_EXECUTE, &CWizardAddGameBoard::OnBnClickedExecute)
	ON_EN_CHANGE(IDC_DEEP, &CWizardAddGameBoard::OnEnChangeDeep)
	ON_WM_DESTROY()
	ON_MESSAGE(WM_MSG, OnMessage)
	ON_NOTIFY(NM_CLICK, IDC_LIST, &CWizardAddGameBoard::OnNMClickList)
	ON_CBN_KILLFOCUS(IDC_COMBO1, &CWizardAddGameBoard::OnCbnKillfocusCombo1)
	ON_CBN_CLOSEUP(IDC_COMBO1, &CWizardAddGameBoard::OnCbnCloseupCombo1)
	ON_NOTIFY(NM_CUSTOMDRAW, IDC_LIST, &CWizardAddGameBoard::OnNMCustomdrawList)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST, &CWizardAddGameBoard::OnLvnColumnclickList)
	ON_NOTIFY(LVN_GETDISPINFO, IDC_LIST, &CWizardAddGameBoard::OnLvnGetdispinfoList)
	ON_REGISTERED_MESSAGE(i8desk::g_nOptApplyMsg, OnApplyMessage)
END_MESSAGE_MAP()

BOOL CWizardAddGameBoard::OnInitDialog()
{
...
void CWizardAddGameBoard::OnBnClickedBrowseDir()
{
	BROWSEINFO info;
...
void CWizardAddGameBoard::ScanDir(const CString& dir, std::vector<tagItem>& lstDir, int nDeep, int nCurDeep)
{
	if (nCurDeep == nDeep)
		return ;
...
void CWizardAddGameBoard::OnBnClickedExecute()
{
	CString cmd;
...
void CWizardAddGameBoard::OnEnChangeDeep()
{
...
long CWizardAddGameBoard::GetGameIdFromIdx(const CString& dir)
{
...
*/
UINT __stdcall CWizardAddGameBoard::WorkThread(LPVOID lpVoid)
{
	CoInitialize(NULL);
	CWizardAddGameBoard* pDlg   = reinterpret_cast<CWizardAddGameBoard*>(lpVoid);
...
void CWizardAddGameBoard::OnDestroy()
{
	SetEvent(m_hExited);
	if (m_hThread != NULL)
	{
...
LRESULT CWizardAddGameBoard::OnMessage(WPARAM wParam, LPARAM lParam)
{
...
std::string CWizardAddGameBoard::GetAIDByName(std::string& Name)
{
...
BOOL CWizardAddGameBoard::ParaseAreaRTTexT(CString szText, i8desk::tagGameRunType& RtType)
{
...
LRESULT CWizardAddGameBoard::OnApplyMessage(WPARAM wParam, LPARAM lParam)
{
	return TRUE;
}

void CWizardAddGameBoard::OnBnClickedOk()
{
	std::string ErrInfo;
...
void CWizardAddGameBoard::OnNMClickList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CWizardAddGameBoard::OnCbnKillfocusCombo1()
{
...
void CWizardAddGameBoard::OnCbnCloseupCombo1()
{
	OnCbnKillfocusCombo1();
}

void CWizardAddGameBoard::OnNMCustomdrawList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CWizardAddGameBoard::OnLvnColumnclickList(NMHDR *pNMHDR, LRESULT *pResult)
{
	*pResult = 0;
...
void CWizardAddGameBoard::OnLvnGetdispinfoList(NMHDR *pNMHDR, LRESULT *pResult)
{
