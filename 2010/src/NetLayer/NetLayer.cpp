...
namespace i8desk
{
	...
	void CNetLayer::Stop()
	{
		try
		{
			...
			CAutoLock<CLock> lock(&m_OptexClient);
	...
	DWORD CNetLayer::ParasePackage(char* package, DWORD length)
	{
		...
		if (length >= sizeof(_packageheader))
		{
			_packageheader* header = (_packageheader*)package;
	...
	{
		{
			...
			_stprintf(buf, TEXT("netlayer:accept:%d:count:%d\r\n"), pHandle->socket, m_mapClient.size());
			OutputDebugString(buf);
	#endif
			{
	...
	#ifdef _DEBUG
				TCHAR buf[MAX_PATH] = {0};
				_stprintf(buf, TEXT("netlayer:close:%d:count:%d\r\n"), sck, m_mapClient.size());
				OutputDebugString(buf);
	#endif
			}
		}
...
// 				CAutoLock<CLock> lock(&pNetlayer->m_OptexClient);
