...
//虚拟盘的两个接口函数.
typedef DWORD (WINAPI *PFNDISKMOUNT)(DWORD /*index*/, DWORD addr, WORD port, char driver, LPCSTR tempfile);
...
HICON CDbMgr::FindIconOfGame_(DWORD gid)
{
	...
	if (m_pIconInfo != NULL && it != m_mapIconInfo.end() && it->second != NULL)
	{
		...
		CIcon ico;
		if(ico.CreateFromBuff((LPBYTE)pData, dwSize))
		{ 
			hIcon = ico.GetHICON();
		}
	}
...
void CDbMgr::LoadSysOpt()
{
	...
	try
	{
		...
		if (CMD_RET_SUCCESS == package.popDWORD())
		{
			...
			{	
				char szName[DEFAULT_FILED_LENGTH] = {0}, szValue[DEFAULT_FILED_LENGTH] = {0};
				CString Val;
				package.popString(szName); 
				Val = szName; Val.MakeLower();
				package.popByte();
	...
	try
	{
		...
		if (CMD_RET_SUCCESS == package.popDWORD())
		{
			...
			for (DWORD idx=0; idx<dwCount; idx++)
			{
				...
				char szValue[DEFAULT_FILED_LENGTH] = {0};
				package.popString(szValue);	pVDisk->VID = szValue;
				...
				if (bHaveSID)
				{
					package.popString(szValue); pVDisk->SID = szValue;
				}
	...
	try
	{
		...
		if (CMD_RET_SUCCESS == package.popDWORD())
		{
			...
			for (DWORD idx=0; idx<dwCount; idx++)
			{
				...
				package.popString(szValue);	pGame->Name			= szValue;
				GetGameNamePy(pGame);
				package.popString(szValue);	pGame->IdcClass		= szValue;
				
				gameClass.insert(std::make_pair(pGame->IdcClass, pGame->IdcClass));

				package.popString(szValue); pGame->Exe			= szValue;
				package.popString(szValue); pGame->Param		= szValue;
				pGame->Size = package.popDWORD();
				pGame->DeskLnk = (package.popDWORD() != 0);
				pGame->Toolbar = (package.popDWORD() != 0);
				package.popString(szValue);	pGame->SvrPath		= szValue;
				package.popString(szValue);	pGame->CliPath      = szValue;
				pGame->Priority = package.popDWORD();
				pGame->ShowPriority = package.popDWORD();
				package.popString(szValue);	pGame->MatchFile	= szValue;
				package.popString(szValue);	pGame->SaveFilter	= szValue;
				package.popDWORD();			//游戏在中心服务器的更新时间
				package.popDWORD();			//游戏在网吧服务器的更新时间
				...
				package.popDWORD();			//游戏添加时间
				package.popString(szValue);	pGame->Url			= szValue;
				pGame->RunType = package.popDWORD();
				package.popDWORD();			//游戏是否更新
				package.popDWORD();			//游戏是否显示
				package.popString(szValue);	pGame->VID			= szValue;
...
void CDbMgr::LoadIconInfo()
{
	...
	CString File = GetDataDir() + TEXT("gameicon.zip");
	HANDLE hFile = CreateFile(File, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, NULL);
	if (hFile == INVALID_HANDLE_VALUE)
	{
		AfxWriteLog2(TEXT("Open Icon Info file Fail:%d"), GetLastError());
		return ;
	}
	...
	{
		AfxWriteLog2(TEXT("Read Icon Info file Fail:%d"), GetLastError());
		CloseHandle(hFile);
		delete[] m_pIconInfo;
		m_pIconInfo = NULL;
		return ;
	...
	try
	{
		...
		while (dwLastPackage == 0 && pTemp < m_pIconInfo + dwSize)	//dwLastPackage=1代表最后一包
		{
			...
			for (DWORD idx=0; idx<dwCount; idx++)
			{
				...
				m_mapIconInfo.insert(std::make_pair(atoi(gid), pTemp + nOffset));
	...
	else
	{
		...
		try
		{
			...
			if (CMD_RET_SUCCESS == package.popDWORD())
			{
				...
				{
					char buf[DEFAULT_FILED_LENGTH] = {0};
...
UINT CDbMgr::ThreadProc(LPVOID lpVoid)
{
	...
	HMODULE hMod = LoadLibrary(szIniFile);
	PFNDISKMOUNT pfnDiskMount = (PFNDISKMOUNT)GetProcAddress(hMod, "iDiskMount");
	if (pfnDiskMount == NULL)
	{
	...
	for (; it != pDbMgr->m_mapVDisk.end(); it++)
	{
		...
		if (pVDisk->Type == 0 && (pVDisk->LoadType == vdStartMenu || pVDisk->LoadType == vdBoot))	//表示是i8自己的虚拟盘
		{
			...
			{
				int ret = pfnDiskMount(0, ip, port, (char)pVDisk->CliDrv, _bstr_t(szVDiskTempDir));
...
void CRptData::ReprotDataToServer(CDockNavBar* pNav, std::map<DWORD, DWORD>& GameClick, bool bAck /* = true */)
{
	...
	if (ConnectToServer())
	{
		...
		pheader->Cmd		= CMD_GAME_REPCLICK;
		pheader->Length		= 0;
		pheader->Version	= PRO_VERSION;
		pheader->Resv[0]	= OBJECT_BARONLINE;
		...
		//导航栏的点击数（包括菜单点击数）
		char szMacAddr[30] = {"FF-FF-FF-FF-FF-FF"};
		...
		pheader->Length = package.GetOffset();

		//清空己报过的游戏点击数。
		if (SendDataToServer(buf, pheader->Length, bAck))
		{
			GameClick.clear();
		}
		CloseServer();
...
bool CRptData::SendDataToServer(char* pData, int len, bool bAck /* = false */)
{
	...
	while (1)
	{
		...
		if (FD_ISSET(m_sck, &fdset))
		{
			...
			if (nRecvLen >= sizeof(_packageheader) && nRecvLen >= (int)pheader->Length)
				return true;
		}
	}
	return true;
