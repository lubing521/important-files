...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_REGISTERED_MESSAGE(WM_HTTP_ERROR_MESSAGE,    HandleHttpError)
 	ON_WM_MEASUREITEM()
 	ON_WM_DRAWITEM()
	ON_WM_TIMER()
...
void CMainFrame::CreateControl()
{
	...
	//设置listctrl的属性
	{
		...
		m_pbmpLstTop = AfxGetSkinImage(TEXT("Skin/Wnd/背景_图标上部间隔.png"));
	...
	case FWM_GAME:
		{
			...
			if (m_pbmpLstBk != NULL)
			{
				...
				AfxDrawImage(&dcMem, m_pbmpLstBk, CRect(0, 0, nImgWidth, nImgHeight));
...
//	pDC->FillRect(&rc, &CBrush(RGB(255, 255, 255)));//&CBrush(0xFFDE95));
	AfxDrawImage(pDC, m_pbmpLstTop, rc);
