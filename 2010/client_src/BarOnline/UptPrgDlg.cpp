...
void CUptPrgDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	{
		if (m_nCurrentUptIdx < m_nGameCount 
			&&m_arGameInfo[m_nCurrentUptIdx].RunType == 1) //本地更新的游戏显示进度
		{
			if (m_hUpdate != NULL)
			{
				CString str;
				...
				{
					m_btnProgress.SetProgres(0);
					m_btnStatus.SetWindowText(TEXT("空间不足!"));
					m_btnTime.SetWindowText(TEXT(""));
					FastCopy_End(m_hUpdate);
					m_hUpdate = NULL;
					m_bIsUpdate = false;
				...
				else if (FCS_PROGRESS_UPDATE == Status.Progress)
				{
					...
					str.Format(TEXT("正在更新:[%s][%dMB/%dMB, %dK/S]"), (LPCTSTR)m_arGameInfo[m_nCurrentUptIdx].Name, 
...
BOOL CUptPrgDlg::OnEraseBkgnd(CDC* pDC)
{
	CRect rc;
	GetClientRect(&rc);
	AfxDrawImage(pDC, TEXT("Skin/Wnd/GongGao/背景_更新.png"), rc);
	return TRUE;
}
