...
namespace http
{
	...
	template < typename CallbackT >
	inline Buffer DownloadFile2(const stdex::tString &url, const CallbackT &callback)
	{
        const int C_INFO_MAX = 1000;
		URL_COMPONENTS urlComp		= {0};
		TCHAR host[C_INFO_MAX]		= {0};
		TCHAR req[C_INFO_MAX]		= {0};
		...
		utility::CAutoWinHttp hSession = ::WinHttpOpen(L"A WinHTTP I8Desk download Program/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, 
		...
		Buffer buf = MakeBuffer(dwTotal + 1);
