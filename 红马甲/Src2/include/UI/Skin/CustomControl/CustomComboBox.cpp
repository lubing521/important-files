...
void CCustomComboBox::SetBk(HBITMAP bk, HBITMAP editBmp)
{
    ...
    editBmp_ = editBmp;
    bk_.Attach(ui::draw::CopyBitmap(bk));
}
...
void CCustomComboBox::SetArrow(HBITMAP normalArrow, HBITMAP hoverArrow, HBITMAP downArrow)
{
	...
	bmpArrow_[0].Attach(ui::draw::CopyBitmap(normalArrow));
	bmpArrow_[1].Attach(ui::draw::CopyBitmap(hoverArrow));
	bmpArrow_[2].Attach(ui::draw::CopyBitmap(downArrow));
}
