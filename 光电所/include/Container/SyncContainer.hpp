...
namespace async
{
	namespace container
	{
		...
		public:
			...
			template < typename OP >
			void op_if(const key_type &key, const OP &op) const
			{
				AutoLock lock(mutex_);
			...
			template < typename OP >
			void not_if_op(const key_type &key, const OP &op) const
			{
				AutoLock lock(mutex_);
				const_iterator iter = container_.find(key);
				if( iter == container_.end() )
			...
			template < typename OP >
			void op_if(const key_type &key, OP op)
			{
				AutoLock lock(mutex_);
			...
			template < typename OP >
			void not_if_op(const key_type &key, OP op)
			{
				AutoLock lock(mutex_);
				iterator iter = container_.find(key);
				if( iter == container_.end() )
