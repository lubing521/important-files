...
namespace utility 
{
	...
	inline const stdex::tString &GetLocalIP()
	{
		static struct GetLocalIP
		{
		public:
			stdex::tString m_strLocalIP;
			...
			GetLocalIP()
			{
				...
				if( ERROR_SUCCESS == ::GetAdaptersInfo(info, &dwSize) )
				{
					...
					while (pAdapter != NULL)
					{
						...
						while (pAddr != NULL)
						{
							m_strLocalIP += CA2T(pAddr->IpAddress.String);
							pAddr = pAddr->Next;
		...
		return LocalIP.m_strLocalIP;
	}

	inline void GetLocalIPs(std::vector<stdex::tString> &localIPs)
	{
		IP_ADAPTER_INFO info[16] = {0};
		DWORD dwSize = sizeof(info);
		if( ERROR_SUCCESS == ::GetAdaptersInfo(info, &dwSize) )
		{
			...
			while (pAdapter != NULL)
			{
				...
				while (pAddr != NULL)
				{
					localIPs.push_back((LPCTSTR)CA2T(pAddr->IpAddress.String));
					pAddr = pAddr->Next;
				}
	...
	//在i8desk名字空间里除了用FILE_Ptr代替FILE*外，其他一切使用方式不变
	//FILE_Ptr保存打开的文件指针，在退出作用域时自动会关闭文件
	//如果要立即关闭文件，调用fclose

	typedef std::tr1::shared_ptr<FILE> FILE_Ptr;

	inline FILE_Ptr fopen(const stdex::tString& filename, LPCTSTR mode)
	{
		FILE *f = NULL;
        ::_tfopen_s(&f, filename.c_str(), mode);
		if (f == NULL) {
			return FILE_Ptr();
		}
	...
	inline FILE_Ptr fopen(LPCTSTR filename, LPCTSTR mode)
    {
        FILE *f = NULL;
        ::_tfopen_s(&f, filename, mode);
		if (f == NULL) {
			return FILE_Ptr();
		}
		return FILE_Ptr(f, &::fclose);
	...
	inline void fclose(FILE_Ptr& f)
	{
		f.reset();
	}
	inline int fseek(FILE_Ptr& f, long _Offset, int _Origin)
	{
		return ::fseek(&*f, _Offset, _Origin);
	}
	inline long ftell(FILE_Ptr& f)
	{
		return ::ftell(&*f);
	}
	inline size_t fread(void *_DstBuf, size_t _ElementSize, size_t _Count, FILE_Ptr& f)
	{
		return ::fread(_DstBuf, _ElementSize, _Count, &*f);
	}
	inline size_t fwrite(const void *_Str, size_t _Size, size_t _Count, FILE_Ptr& f)
	{
		return ::fwrite(_Str, _Size, _Count, &*f);
	}
	inline int fflush(FILE_Ptr& f)
	{
		return ::fflush(&*f);
	}
	inline int fgetc(FILE_Ptr& f)
	{
		return ::fgetc(&*f);
	}
	inline int fgetpos(FILE_Ptr& f, fpos_t *_Pos)
	{
		return ::fgetpos(&*f, _Pos);
	}
	inline char* fgets(char *_Buf, int _MaxCount, FILE_Ptr& f)
	{
		return ::fgets(_Buf, _MaxCount, &*f);
	}
	inline int fputc(int _Ch, FILE_Ptr& f)
	{
		return ::fputc(_Ch, &*f);
	}
	inline int fputs(const char *_Str, FILE_Ptr& f)
	{
		return ::fputs(_Str, &*f);
	}
	...
	inline BOOL mkpath(const std::string &path)
	{
		...
		if( *(path.rbegin()) != '\\' )
			return ::SHCreateDirectoryExA(NULL, (path + "\\").c_str(), 0);
		else
			return ::SHCreateDirectoryExA(NULL, path.c_str(), 0);
	}

	inline BOOL mkpath(const wchar_t *path)
	{
		return mkpath(std::string(CW2A(path)));
	}

	template<typename CharT>
	inline BOOL mkpath(const std::basic_string<CharT> &path)
	{
		return mkpath(path.c_str());
	}

    inline BOOL MakeSureDirectoryPathExists(LPCTSTR lpszFilePath)
    {
        return ::MakeSureDirectoryPathExists(CT2A(lpszFilePath));
    }
	...
	inline bool WriteFileContent(HANDLE hFile, const BYTE *buf, DWORD len)
	{
		if (len == 0)
			return true;
	...
	// Process
	inline DWORD GetProcessIdByName(LPCTSTR lpszProcName)
	{
		utility::CAutoHandle hSnap = ::CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
	...
	inline HANDLE GetTokenFromIE()
	{
		...
		{ 
			do 
			{
				if( utility::Strcmp(pe32.szExeFile, _T("explorer.exe")) == 0 )
				{
	...
	inline bool StartProcess(LPCTSTR lpszName, LPCTSTR lpszCmdLine)
	{
		utility::CAutoHandle hToken = GetTokenFromIE();
		LPVOID lpEnv = NULL;
		if ( hToken.IsValid() )
        ...
        TCHAR atCmdLine[1000];
        _tcscpy_s(atCmdLine, lpszCmdLine);
		if( ::CreateProcessAsUser(hToken, lpszName, atCmdLine, NULL, NULL, FALSE, CREATE_UNICODE_ENVIRONMENT, lpEnv, NULL, &si, &pi))
		{
	...
	inline bool KillService(LPCTSTR lpszService)
	{
