...
typedef struct _CACHEOBJECT
{
	...
	LARGE_INTEGER      NonPaeCache;		// 普通内存缓冲上限
	LARGE_INTEGER      NonPaeCacheUsed;	 // 已经使用的普通内存大小
	LARGE_INTEGER      PaeCache;	   // 分配的pae内存大小
	HANDLE     ReadEvent;
	HANDLE     WriteEvent;
...
typedef VOID (__stdcall *PFNCLEARCACHE)(PCACHEOBJECT cacheobj);//////////////////////////////////////////////////////////////////////////
...
namespace i8desk
{
	...
	public:
		...
		struct PER_HANLDE_CONTENT
		{
			SOCKET socket;
			sockaddr_in	addr;
			char cliName[MAX_NAME_LEN];	// 客户机名字
			ulong		speed;			// 速度
			ulonglong	total;			// 总量
			ulonglong	oldTotal;		// 上一次总量
			ulong		maxData;		// 峰值
			ulong		disTime;		// 两次读取间隔时间
			ulong		oldTime;		// 上一次时间
			ulong		firstTime;		// 起始时间
			ulong		lastTime;		// 当前连接时间
			ulong		cacheCounter;	// 缓存命中数
			ulong		systemCounter;	// 系统读取数
	...
	public:
		...
		virtual void Release() { delete this; }
	public:
		virtual DWORD Start(LPCSTR lpszIp, DWORD wPort, LPCSTR lpszFileName, ulong cacheSize, wchar_t ssd);
		virtual void  Stop();
		...
		BOOL GetVDiskInfoEx(VDiskClientStatus **vDiskClient, size_t &cnt)
		{
			CObjectLock lock(m_OptexClient);
			
			size_t size = m_mapClient.size() * sizeof(VDiskClientStatus);
			if( size == 0 )
				return FALSE;

			VDiskClientStatus *tmp = (VDiskClientStatus *)::CoTaskMemAlloc(size);

			size_t i = 0;
			for(std::map<SOCKET, PER_HANLDE_CONTENT*>::const_iterator iter = m_mapClient.begin();
				iter != m_mapClient.end(); ++iter)
			{
				memset(tmp + i, 0, sizeof(VDiskClientStatus));

				tmp[i].ClientIP	= iter->second->addr.sin_addr.s_addr;
				//_tcscpy_s(tmp[i].ClientName, MAX_NAME_LEN, CA2T(iter->second->cliName));
				tmp[i].ReadCount	= iter->second->total;
				tmp[i].ReadSpeed	= iter->second->speed;
				tmp[i].ReadMax		= iter->second->maxData;
				tmp[i].ConnectTime	= iter->second->lastTime;
				tmp[i].CacheShooting = iter->second->cacheCounter * 100.0 / iter->second->systemCounter;
		...
		m_pfnClear(0)*/
	{
		...
		//m_pfnRelease	   = (PFNRELEASE)GetProcAddress(hModCache, "ReleaseCache");
		//m_pfnClear		   = (PFNCLEARCACHE)GetProcAddress(hModCache, "ClearCache");

		/*if (!m_pfnOpenPartition || !m_pfnReadData || !m_pfnRelease || !m_pfnClear)
		{
			m_pfnOpenPartition = NULL;
			m_pfnReadData	   = NULL;
			m_pfnRelease	   = NULL;
	...
	DWORD CNetLayer::Start(LPCSTR lpszIp, DWORD wPort, LPCSTR lpszFileName, ulong cacheSize, wchar_t ssd)
	{
		::OutputDebugStringA("开始启动...");
		g_sock = INVALID_SOCKET;
		m_wPort = wPort;
		if (lpszIp != NULL)
			lstrcpyA(m_szIp, lpszIp);
		else
			m_szIp[0] = 0;
		...
		::OutputDebugStringA("创建DiskFile");
		DWORD dwError = CreateDiskFile(lpszFileName);
		if (dwError != ERROR_SUCCESS)
			return dwError;

		::OutputDebugStringA("启动监听socket...");
		...
		/*try
		{
			::OutputDebugStringA("创建缓存...");
			if (m_pfnOpenPartition != NULL && m_pfnReadData != NULL && m_pfnRelease != NULL && m_pfnClear != 0 )
			{
				m_pCacheObject = new _CACHEOBJECT;
				ZeroMemory(m_pCacheObject, sizeof(_CACHEOBJECT));
				m_pCacheObject->tempFile = NULL;
				m_pCacheObject->PartitionLetter = lpszFileName[4];
				m_pCacheObject->CacheLeft.QuadPart = cacheSize * 1024LL * 1024LL;
				m_pCacheObject->DriverSize = m_nFileSize;
				m_pCacheObject->SSDDriverLetter = ssd;
				if (m_pfnOpenPartition(m_pCacheObject))
				{
					...
					::OutputDebugString(_T("成功完成..."));
			}
		}
		catch (...) { delete m_pCacheObject; m_pCacheObject = NULL; }*/

		::OutputDebugStringA("成功完成...");
	...
	DWORD CNetLayer::StartListenSocket()
	{
		...
		addr.sin_port = htons((u_short)m_wPort);
// 		if (lstrlen(m_szIp))
...
_do_err:
	...
	DWORD CNetLayer::CreateDiskFile(LPCSTR lpFileName)
	{
		...
		m_hFile2 = CreateFileA(lpFileName, 
			GENERIC_READ | GENERIC_WRITE,
			FILE_SHARE_READ | FILE_SHARE_WRITE,
			NULL, 
			OPEN_EXISTING,
		...
		ov.hEvent = CreateEvent(NULL, TRUE, FALSE, _T("Global\\VDISK_DSCSISERVERINIT"));
		if( ov.hEvent == NULL )
			return GetLastError();
	...
	{
		{
			...
			pHandle->disTime = GetTickCount();
			pHandle->firstTime = ::GetTickCount();
			pHandle->lastTime = 0;
			pHandle->speed = 0;
			pHandle->total = 0;
			pHandle->oldTotal = 0;
	...
	{	
...

		{
			...
			DWORD dwDiff = curTime - pHandle->disTime;
			pHandle->disTime += dwDiff;
			pHandle->lastTime = curTime - pHandle->firstTime;

			if( pHandle->disTime - pHandle->oldTime >= 1000 )
			{
				pHandle->speed = (ulong)(pHandle->total - pHandle->oldTotal);
				pHandle->oldTotal = pHandle->total;
				pHandle->oldTime = pHandle->disTime;
			}
			
			if( pHandle->maxData < pHandle->speed )
				pHandle->maxData = pHandle->speed;
			...
			case EVENT_ACCEPT:
				{
					...
					pHandle->cacheCounter = 0;
					pHandle->systemCounter = 0;
	...
	void CNetLayer::Handle(PER_HANLDE_CONTENT*pHandle, PER_IO_CONTEXT* pIoData)
	{
		...
		case DRP_MJ_REFRESH:
			{
				OutputDebugStringA("I8VDisk:Recv Refresh Vdisk.");
				RefreshBufferEx();
				...
				OutputDebugStringA("I8VDisk:Ack Refresh Vdisk.");
			}
			break;
		case DRP_MJ_QUERY:
			{
		...
		case DRP_MJ_READ:
			{
				...
				++(pHandle->systemCounter);
				//if (bNeedReadFile && m_pCacheObject != NULL)
				//{
				//	if (FALSE == m_pfnReadData(m_pCacheObject, pDrp->Read.Offset, lpStatus->buffer, pDrp->Read.Length))
				//	{
				//		/*char szLog[MAX_PATH] = {0};
	...
	DWORD WINAPI StartServer(PHANDLE handle, LPCSTR lpszSvrIP, TCHAR szDriver, DWORD wPort, ulong cacheSize, wchar_t ssd)
	{
		CNetLayer* pserver = new CNetLayer;
		char szFileName[MAX_PATH] = {0};
		sprintf_s(szFileName, "\\\\.\\%C:", szDriver);
		DWORD dwError = pserver->Start(lpszSvrIP, wPort, szFileName, cacheSize, ssd);
	...
	void WINAPI GetVdiskInfoEx(HANDLE handle, VDiskClientStatus **vDiskInfo, size_t *cnt)
	{
		if (handle == NULL) 
			return ;
		try
		{
			CNetLayer* pServer = (CNetLayer*)handle;
			pServer->GetVDiskInfoEx(vDiskInfo, *cnt);
		}
		catch (...) { }
	}
	...
	class CVDiskPlug : public ISvrPlug
	{
	...
	public:
		...
		virtual LPCTSTR GetPlugName()	{ return PLUG_VDISK_NAME; }
	...
	bool CVDiskPlug::Initialize(i8desk::ISvrPlugMgr *pPlugMgr)
	{
		m_pPlugMgr = pPlugMgr;
	...
	DWORD	CVDiskPlug::SendMessage(DWORD cmd, DWORD param1, DWORD param2)
	{
		...
		case VDISK_CMD_START:
			{
				...
				HANDLE  handle = NULL;
				StartServer(&handle, "", pVDInfo->szSvrDrv, pVDInfo->nPort, pVDInfo->cacheSize, pVDInfo->ssdDrv);
				if (handle != NULL)
				{
					tagVDInfo* pTemp = new tagVDInfo;
					*pTemp = *pVDInfo;
					m_lstVDisk.insert(std::make_pair(handle, pTemp));
				}
				return (DWORD)handle;
			}
			break;
		...
		case VDISK_CMD_GETINFO:
			...
			break;
		case VDISK_CMD_GETINFOEX:
			{
				HANDLE handle = (HANDLE)param1;
				VDiskClientStatus **pVDInfo = (VDiskClientStatus**)param2;

				size_t cnt = 0;
				GetVdiskInfoEx(handle, pVDInfo, &cnt);
