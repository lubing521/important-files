...
namespace i8desk
{
	...
	private:
		...
		ILogger *log_;
		std::vector<stdex::tString> fileList_;		// 需要更新的文件列表
	...
	private:
		...
		{
			if( getFileImpl_->IsRunOnMainServer() )
				return 0;
			...
			while( !thread_.IsAborted() )
			{
				...
				try
				{
					...
					{
						log_->WriteLog(LM_INFO, _T("开始重启服务"));
						_RestartService();
					}
				...
				{
					log_->WriteLog(LM_ERROR, CA2T(e.what()));
				}
				catch(...)
				{
					log_->WriteLog(LM_ERROR, _T("更新程序未知异常: %d"), ::GetLastError());
				}
