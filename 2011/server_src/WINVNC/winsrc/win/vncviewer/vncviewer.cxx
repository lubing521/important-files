...
void
processParams(const char* pcParam)
{
    try {
        // Try to process <option>=<value>, or -<bool>
        if (Configuration::setParam(pcParam, true))
            return;

        // If it's <host> then add it to the list to connect to.
        hosts.push_back(strDup(pcParam));
...
// -=- main
//
BOOL WINAPI __stdcall VNC_Run(const char *pcSvrIP, const char* pcPassword)
{
    if (strcmp(pcPassword, "Ke93Jr7Ud30Pd4Fn7EqQ3Lmus62LmDEnbc5ezx") != 0)
    {
        return FALSE;
    }
  try {
