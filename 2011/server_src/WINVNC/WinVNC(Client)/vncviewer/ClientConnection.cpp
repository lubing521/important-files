...
extern char g_szAddr[17];
extern char g_szPsw[64];
extern int g_nPort;
...
//		AppendMenu(hsysmenu, MF_STRING, IDC_OPTIONBUTTON,	_T("连接配置..."));
		AppendMenu(hsysmenu, MF_STRING, ID_CONN_ABOUT,		_T("连接信息"));
		AppendMenu(hsysmenu, MF_STRING, ID_REQUEST_REFRESH,	_T("刷新屏幕"));

		AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
		AppendMenu(hsysmenu, MF_STRING, ID_FULLSCREEN,		_T("全屏显示"));
		AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
 		AppendMenu(hsysmenu, MF_STRING, ID_CONN_CTLALTDEL,	_T("发送 Ctl-Alt-Del"));
		...
		AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
		AppendMenu(hsysmenu, MF_STRING, ID_NEWCONN,			_T("新建连接..."));
		AppendMenu(hsysmenu, MF_STRING | (m_serverInitiated ? MF_GRAYED : 0), 
			ID_CONN_SAVE_AS,	_T("保存连接信息..."));
	}
    AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
	AppendMenu(hsysmenu, MF_STRING, IDD_APP_ABOUT,		_T("关于 VNCviewer..."));
	if (m_opts.m_listening) {
		AppendMenu(hsysmenu, MF_SEPARATOR, NULL, NULL);
		AppendMenu(hsysmenu, MF_STRING, ID_CLOSEDAEMON, _T("关闭监听"));
	}
	DrawMenuBar(m_hwnd);
...
void ClientConnection::GetConnectDetails()
{
	...
	else 
	{
		...
		_tcsncpy_s(m_host, g_szAddr, MAX_HOST_NAME_LEN);
		m_port = g_nPort;
		//_tcsncpy(m_host, "192.168.159.87", MAX_HOST_NAME_LEN);
		//m_port = 5900;
	}
...
void ClientConnection::Connect()
{
	...
	if (res == SOCKET_ERROR) throw WarningException("连接客户端失败");
