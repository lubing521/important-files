		...
		else	//file.
		{
			...
			if (_wcsicmp(subfile.c_str(), INDEX_FILE_NAMEW) != 0)
			{
				...
				MSXML2::IXMLDOMElementPtr FileNode = xmlDoc->createElement("f");
				{
					...
					if (-1 != t)
					{
						swprintf_s(buf, L"%X", t);
						FileNode->setAttribute("t", buf);
					}
					else
					{
						swprintf_s(buf, L"I64X", MAKEQWORD(wfd.ftLastWriteTime.dwLowDateTime, 
					...
					{
						swprintf_s(buf, L"%X", wfd.dwFileAttributes);
		   ...
		   const std::wstring& dir, int blksize = INDEX_BLOCK_SIZE)
{
	...
	for (long idx=0; idx<list->Getlength(); idx++)
	{
		...
		else
		{
			...
			utility::CAutoFile hFile = CreateFileW(file.c_str(), GENERIC_READ, FILE_SHARE_READ|FILE_SHARE_WRITE, 
				NULL, OPEN_EXISTING, 0, NULL);
			if (!hFile.IsValid())
				return GetLastError();

			LARGE_INTEGER qFileSize = {0};
			if (!GetFileSizeEx(hFile, &qFileSize))
				return GetLastError();

			if( qFileSize.QuadPart == 0 )
				continue;
			...
			if( qFileSize.QuadPart < INDEX_BLOCK_SIZE )
				dwBlockBytes = (DWORD)qFileSize.QuadPart;
			else
				dwBlockBytes = INDEX_BLOCK_SIZE;

			DWORD dwFileCRC = -1;
			while (qwFileOffset < qFileSize.QuadPart )
			{
				if( qFileSize.QuadPart - qwFileOffset < INDEX_BLOCK_SIZE )
					dwBlockBytes = static_cast<DWORD>(qFileSize.QuadPart - qwFileOffset);
				...
				DWORD blkCRC = -1;
				for (DWORD idx=0; idx<dwBlockBytes; idx++)
				{
					BYTE tmp = *((char *)lpbMapAddress.Get() + idx);
					CalcCrc32(tmp, dwFileCRC);
					CalcCrc32(tmp, blkCRC);
				}
				blkCRC = ~blkCRC;
				tagIdxBlock block = {0};
				block.blksize = dwBlockBytes;
				block.crc     = blkCRC;
...
{
public:
	CPlParser(IdxMap& mF, std::vector<tagIdxFile*>& vF, DWORD& crcoffset) 
	...
	IDXCALLBACK pfnCallback, void* pCookie)
{
	...
	for (std::vector<tagIdxFile*>::iterator it = vFiles.begin(); it != vFiles.end(); it ++)
	{
		...
		if (!(pFile->attr & FILE_ATTRIBUTE_DIRECTORY))
		{
			...
			swprintf_s(file, L"%s%s", root, pFile->name.c_str());
...
{
	qSize = 0;
	for (IdxMap::iterator first = mNewFiles.begin(); first != mNewFiles.end(); first ++)
	{
		tagIdxFile* pFile = first->second;
		if (pfnCallback != NULL && !pfnCallback(pCookie, pFile->name.c_str()))
			break;
		IdxMap::iterator second = mOldFiles.find(first->first);
...
void Idxfile_DeleteMorefile(const std::wstring& dir, int baselen, IdxMap& mFiles, IDXCALLBACK pfnCallback, void* pCookie)
{
	WIN32_FIND_DATAW wfd = {0};
	HANDLE hFinder = FindFirstFileW((dir + L"*.*").c_str(), &wfd);
	if (hFinder == INVALID_HANDLE_VALUE)
	...
	do 
	{
		...
		else
		{
			std::wstring file	= dir + wfd.cFileName;
			//delte more file.
			if (mFiles.end() == mFiles.find(file.substr(baselen)) && _wcsicmp(wfd.cFileName, L"i8desk.idx") != 0)
			{
...
void Idxfile_DeleteMorefile(LPCWSTR dir, IdxMap& mFiles, IDXCALLBACK pfnCallback, void* pCookie)
{
