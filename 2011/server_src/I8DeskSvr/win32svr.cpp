...
namespace i8desk
{
	...
	CWin32Svr::CWin32Svr(LPCTSTR lpszSvrName, LPCTSTR lpszSvrDispName,LPCTSTR lpszSvrDesc, DWORD dwControlsAccepted) :
	...
	DWORD CWin32Svr::StopService(LPCTSTR lpszSvrName)
	{
		...
		try 
		{
			...
			if (hSCM == NULL) 
				throw GetLastError();
			hService = OpenService(hSCM, lpszSvrName, SERVICE_STOP | SERVICE_QUERY_STATUS | SERVICE_ENUMERATE_DEPENDENTS);
			if (hService == NULL) 
				throw GetLastError();
	...
	DWORD CWin32Svr::ChangeConfig()
	{
		...
		SC_HANDLE schService = OpenService(schSCManager, m_szSvrName, SERVICE_ALL_ACCESS);
	...
	DWORD CWin32Svr::ReportStatusToSCM(DWORD dwStatus)
	{
		if (m_bDebug)
		{
			...
			_stprintf(buf, TEXT("report service status: %d\r\n"), dwStatus);
	...
	DWORD  CWin32Svr::AddLogToEvent(LPCTSTR fmt, ...)
	{
		...
		va_list marker;
		va_start(marker, fmt);
		int len = _vstprintf(xbuf, fmt, marker);
		va_end(marker);
		LPCTSTR p[1] = {xbuf};
