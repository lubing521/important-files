...
{
public:
	ZCacheFile(const stdex::tString& strFileName, 
		BYTE *data, 
		DWORD size, 
		...
		, m_data(data)
		, m_size(size)
		, m_crc(utility::CalBufCRC32(data, size))
	...
	stdex::tString GetFileName(void) const 
	{
...
public:
	...
	CacheFile_Ptr Open(LPCTSTR lpFileName)
	{
		CacheFile_Ptr f;
		...
		ULONGLONG ftLastWriteTime;
		if (!utility::GetFileLastWriteTime(lpFileName, ftLastWriteTime)) {
			return f;
		}
		...
		BOOL ok = utility::ReadFileContent(hFile, data, size);
		::CloseHandle(hFile);
	...
	void ReloadAll(void)
	{
		...
		typedef std::tr1::unordered_map<stdex::tString, ULONGLONG> CacheMap;
		...
		for (CacheMap::iterator it = files.begin(); it != files.end(); ++it) 
		{
			...
			ULONGLONG ftLastWriteTime;
			if (utility::GetFileLastWriteTime(it->first.c_str(), ftLastWriteTime) 
...
private:
	typedef ZAssociatedContainer<stdex::tString, CacheFile_Ptr,
		std::map<stdex::tString, CacheFile_Ptr>, ZSync> FileCacheList;
