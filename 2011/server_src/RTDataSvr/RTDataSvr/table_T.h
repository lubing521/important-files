...
template<> //For string key, because map<const TCHAR *, Idx> will be incorrect
class ZIndex_T<const TCHAR *>
{
public:
	typedef const TCHAR * Kty;
	void SetIdx(Kty key, uint32 idx) { m_idxs[key].idx = idx; }
	void SetVer(Kty key, ulong ver) { m_idxs[key].ver = ver; }
	uint32 GetIdx(Kty key) { return m_idxs[key].idx; }
	ulong GetVer(Kty key) { return m_idxs[key].ver; }
	...
	{
		size_t n =(_tcslen(key) + 1)*sizeof(Kty);
		Kty p = (Kty)I8MP.alloc(n); 
		memcpy((void *)p, key, n);
		return p;
	}
...
private:
	std::tr1::unordered_map<stdex::tString, Idx> m_idxs; //Kty: const char *
};
...
struct Uint32StringKey {
	...
	typedef uint32 Key1Ty;
	typedef const TCHAR * Key2Ty;

	typedef std::tr1::unordered_map<stdex::tString, Idx> Key2Idx;
	typedef std::tr1::unordered_map<uint32, Key2Idx> Key1Idx;
	...
	static Kty Clone(const Kty& key) 
	{
		...
		size_t n = (_tcslen(key.key2) + 1) * sizeof(Key2Ty);
		k.key2 =(Key2Ty) I8MP.alloc(n); 
...
struct StringUint32Key 
{
	...
	typedef const TCHAR * Key1Ty;
	typedef uint32 Key2Ty;

	typedef std::tr1::unordered_map<uint32, Idx> Key2Idx;
	typedef std::tr1::unordered_map<stdex::tString, Key2Idx> Key1Idx;
	...
	static Kty Clone(const Kty& key) 
	{
		...
		size_t n = (_tcslen(key.key1) + 1)*sizeof(Key1Ty);
		k.key1 = (Key1Ty)I8MP.alloc(n); 
...
struct StringStringKey 
{
	...
	static Kty Clone(const Kty& key) 
	{
		...
		n = (_tcslen(key.key1) + 1)*sizeof(Key2Ty);
		k.key2 = (Key2Ty)I8MP.alloc(n); 
		memcpy((void *)k.key2, key.key2, n);
...
//Load and Save ,可以在实现类中改变缺省行为，如临时表的处理等
	virtual int Load(IDatabase *pDatabase)
	{
		...
		for (uint32 i = 1; i < m_nRecords; i++) {
			...
			Kty key = _This()->GetKey(data);
			m_idxs.SetIdx(key, i);
			m_idxs.SetVer(key, utility::CalBufCRC32(&data, sizeof(Tty)));
	...
	virtual int Load(const stdex::tString& strPath)
	{
		I8_GUARD(ZGuard, Lty, m_lock);

		if (!LoadFromFile(strPath)) {
			Log(LM_INFO, _T("从文件加载表数据失败\n"));
	...
	virtual int Save(const stdex::tString& strPath)
	{
		TCHAR lpszFileName[MAX_PATH];
		::_stprintf(lpszFileName, _T("%s%s"), strPath.c_str(), m_lpszName);

		Log(LM_INFO, _T("保存数据表到文件, %s...\n"), lpszFileName);
		...
		utility::FILE_Ptr f =  utility::fopen(lpszFileName, _T("w+b"));
		if (!f) {
			Log(LM_ERROR,_T("打开文件失败, %s!\n"), lpszFileName);
			return -1;
		}

		uint32 dbversion = DB_VERSION_CUR;
		if (utility::fwrite(&dbversion, 4, 1, f) != 1 //数据库设计版本
			|| utility::fwrite(&m_ulVersion, 4, 1, f) != 1    //表数据内容版本
			|| utility::fwrite(m_pRecords, sizeof(Tty), m_nRecords, f) != m_nRecords) {
			utility::fclose(f);
			Log(LM_ERROR,_T("写入文件内容失败, %s!\n"), lpszFileName);
			return -1;
		}
		utility::fclose(f);
...
//Create Filter Sorter Helper
	Fty *CreateFilter(uint64 CondMask, uint32 CondMode, 
		uint32 dwParam, const stdex::tString& strParam)
	{
		if (CondMode == 0)
			return 0;
...
private:
	struct Filter : public Fty {
		Filter(Impty *pTable, uint64 CondMask, uint32 CondMode, 
			...
			, m_dwParam(dwParam) 
		{
			_tcsncpy(m_szParam, strParam.c_str(), sizeof(m_szParam) - 1);
		}
		...
		assert(optype != SQL_OP_SELECT);
		{
			...
			if (optype == SQL_OP_INSERT || optype == SQL_OP_UPDATE) {
				...
				if (uint32 i = m_idxs.GetIdx(key)) {
					m_idxs.SetVer(key, utility::CalBufCRC32(m_pRecords + i, sizeof(Tty)));
				}
			}
		}
...
private:
	...
	bool LoadFromFile(const stdex::tString strPath)
	{
		//TCHAR szFileName[ MAX_PATH ] = {0};
		//::_stprintf(szFileName, _T("%s%s"), strPath.c_str(), m_lpszName);

		//Log((LM_INFO, _T("从文件加载表数据, %s...\n"), m_lpszName));
		...
		//utility::FILE_Ptr f =  utility::fopen(szFileName, _T("rb"));

		//if (!f) {
		//	Log((LM_WARNING, _T("初始化表[%s]时打开文件[%s]失败!\n"),
		//		m_lpszName, szFileName));

		//	//打开基本文件失败则打开当前版本的文件
		//	_stprintf(szFileName, _T("%s%x\\%s"), strPath.c_str(), DB_VERSION_CUR, m_lpszName);

		//	Log((LM_INFO, _T("从文件加载表数据, %s...\n"), szFileName));
		//	f =  utility::fopen(szFileName, _T("rb"));
		//	if (!f) {
		//		I8_ERROR_RETURN((LM_ERROR, _T("从文件初始化表[%s]失败!\n"),
		//			m_lpszName), false);
		//	}
		...
		//	if (utility::fread(&dbversion, 4, 1, f) != 1 || utility::fread(&m_ulVersion, 4, 1, f) != 1) {
		//		I8_ERROR_RETURN((LM_ERROR, 
		//			_T("初始化表[%s]时从文件读取版本失败!\n"),
		//			m_lpszName), false);
		//	}
		...
		//	utility::fseek(f, 4, SEEK_SET);
		//	utility::fread(&m_ulVersion, 4, 1, f);

		//	if (utility::fread(m_pRecords, sizeof(Tty), m_nRecords, f) != m_nRecords) {
		//		I8_ERROR_RETURN((LM_ERROR, 
		//			_T("初始化表[%s]时从文件读取数据失败!\n"), 
		//			m_lpszName), false);
		//	}
		//}
...
//#define GET_INT_FIELD(Var, Name, Rst)  Var = Rst->GetCollect(Name);
//		//I8_CHK_EXP(Var = Rst->GetCollect(Name);)


//#define GET_STR_FIELD(Var, Name, Rst)\
//		_variant_t tmp = Rst->GetCollect(Name);\
//		if( tmp.vt == VT_EMPTY || tmp.vt == VT_NULL )\
//			return;\
//		_tcsncpy(Var, (_bstr_t)Rst->GetCollect(Name), _countof(Var));
		//I8_CHK_EXP(_tcsncpy(Var, (_bstr_t)Rst->GetCollect(Name), sizeof(Var));)

template<typename T, typename CharT,typename RstT>
inline void GET_INT_FIELD(T &dest, const CharT *name, RstT &rst)
{
	_variant_t tmp = rst->GetCollect(name);
	if( tmp.vt == VT_EMPTY || tmp.vt == VT_NULL )
		return;
...
template<typename CharT, size_t N, typename RstT>
inline void GET_STR_FIELD(CharT (&dest)[N], const CharT *name, RstT &rst)
{
	_variant_t tmp = rst->GetCollect(name);
	if( tmp.vt == VT_EMPTY || tmp.vt == VT_NULL )
		return;

	_tcsncpy(dest, (_bstr_t)rst->GetCollect(name), N);
}

//update record code helper
#define BEGIN_UPDATE() uint64 realmask = 0
template<typename T, typename U>
inline void UPDATE_FIELD(uint64  mask, uint64 MASK, uint64 &realmask, T &lhs, const U &rhs )
{
	if(mask & MASK) 
	{
		if(db::VALUE_FIELD_COMP(lhs, rhs))
		{
			db::VALUE_FIELD_COPY(lhs, rhs);
			realmask |= MASK;
		}
	}
}
...
#define BEGIN_DECLARE_TABLE_CLASS(tname, key) \
	class Z##tname##Table  \
		...
		> \
	{ \
		DECLARE_TABLE(Z##tname##Table, tbl##tname) 
...
#define SQLCOND_STRING_FIELD(urtab, urmn, urfn) \
			...
			return !db::VALUE_FIELD_COMP(szParam, d->urfn);

#define SQLCOND_VALUE_FIELD(urtab, urmn, urfn) \
			case MASK_T##urtab##_##urmn: \
			return !db::VALUE_FIELD_COMP(dwParam, d->urfn);
...
#define SQLSORT_STRING_FIELD(urtab, urmn, urfn) \
		case MASK_T##urtab##_##urmn: \
			return (_tcscmp(d1->urfn, d2->urfn) > 0) == desc;
...
#define INVALIDRECORD_K1_K2_IMPLMENT( fn1,fn2, ty1,ty2) \
	protected: \
	bool InvalidRecord(const Tty& d) \
	{ \
	return !(ty1##_FIELD_VALID(d.fn1)||ty2##_FIELD_VALID(d.fn2)); \
	} 
