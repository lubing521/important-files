...
CI8WndShadow::pfnUpdateLayeredWindow CI8WndShadow::s_UpdateLayeredWindow = NULL;

bool CI8WndShadow::s_bVista = false;
CI8WndShadow::pfnDwmIsCompositionEnabled CI8WndShadow::s_DwmIsCompositionEnabled = NULL;

const TCHAR *strWndClassName = _T("PerryShadowWnd");

HINSTANCE CI8WndShadow::s_hInstance = (HINSTANCE)INVALID_HANDLE_VALUE;

#pragma warning(push)
#pragma warning(disable:4786)
std::map<HWND, CI8WndShadow *> CI8WndShadow::s_Shadowmap;
#pragma warning(pop) 

CI8WndShadow::CI8WndShadow(void)
...
bool CI8WndShadow::Initialize(HINSTANCE hInstance)
{
    ...
    assert(hSysDll != NULL && "¼ÓÔØUSER32.dllÊ§°Ü");
...
void CI8WndShadow::Create(HWND hParentWnd)
{
	...
	Show(hParentWnd, IsWindowVisible(hParentWnd));	// Show the shadow if conditions are met
...
LRESULT CALLBACK CI8WndShadow::ParentProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	_ASSERT(s_Shadowmap.find(hwnd) != s_Shadowmap.end());	// Shadow must have been attached

	CI8WndShadow *pThis = s_Shadowmap[hwnd];
	...
	case WM_DWMCOMPOSITIONCHANGED:
		{
			...
			pThis->Show(hwnd, IsWindowVisible(hwnd));
		}
		break;
...
void CI8WndShadow::Update(HWND hParent)
{
	//int ShadSize = 5;
...
void CI8WndShadow::MakeShadow(UINT32 *pShadBits, HWND hParent, RECT *rcParent)
{
...
bool CI8WndShadow::SetSize(int NewSize)
{
...
bool CI8WndShadow::SetSharpness(unsigned int NewSharpness)
{
	if(NewSharpness > 20)
		return false;
...
bool CI8WndShadow::SetDarkness(unsigned int NewDarkness)
{
	if(NewDarkness > 255)
		return false;
...
bool CI8WndShadow::SetPosition(int NewXOffset, int NewYOffset)
{
...
bool CI8WndShadow::SetColor(COLORREF NewColor)
{
	m_Color = NewColor;
	if(SS_VISABLE & m_Status)
		Update(GetParent(m_hWnd));
...
void CI8WndShadow::Show(HWND hParentWnd, BOOL bShowShadow)
{
